...
class Interpreter
  ...
  #--------------------------------------------------------------------------
  def command_101
    # vérification de la présence d'un message en cours.
    return false if $game_temp.message_proc
    #Initialisation de la variable bloquant l'évent et du proc débloquant l'évent
    @message_waiting = true
    $game_temp.message_proc = Proc.new { @message_waiting = false }
    # construction du message_text
    message_text = @list[@index].parameters[0] + "\n"
    line_count = 1
    loop do
      # Reception des autres lignes, si il y en a trop le message ne risque pas de fonctionner correctement.
      if @list[@index+1].code == 401
        message_text += @list[@index+1].parameters[0] + "\n"
        line_count += 1
      else
        #Partie choix / input number, désactivé pour le moment
=begin
...
=end
        ...
        #Afficher ou continuer un message....
        if $game_switches[101] #Continuer.
          Yuki::ShowMSG.run(message_text,2,y,1000,true,false,$game_switches[102],false,true)
        else
          Yuki::ShowMSG.run(message_text,2,y,1000,false,!$game_switches[103],$game_switches[102],false,true)
        end
        return true
      end
      # インデックスを進める
